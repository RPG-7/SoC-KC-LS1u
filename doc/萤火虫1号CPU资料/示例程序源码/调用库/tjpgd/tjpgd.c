...
)
{
	...
	ndata = (ndata + 3) & (size_t)~3;			/* Align block size to the word boundary */
	...
	while (ndata) {	/* Process all tables in the segment */
		...
		{	/* Create fast huffman decode table */
			...
			if (cls) {
				...
				memset(tbl_ac, (char)0xFF, HUFF_LEN * sizeof (uint16_t));		/* Default value (0xFFFF: may be long code) */
			} else {
				...
				memset(tbl_dc, (char)0xFF, HUFF_LEN * sizeof (uint8_t));		/* Default value (0xFF: may be long code) */
			}
	...
	if (!JD_USE_SCALE || jd->scale != 3) {	/* Not for 1/8 scaling */
		...
		if (JD_FORMAT != 2) {	/* RGB output (build an RGB MCU from Y/C component) */
			for (iy = 0; iy < my; iy++) {
				...
				for (ix = 0; ix < mx; ix++) {
					/* 修改 */
					cb = pc[0] - (short)128; 	/* Get Cb/Cr component and remove offset */
					cr = pc[64] - (short)128;
					if (mx == 16) {					/* Double block width? */
						if (ix == 8) py += 64 - 8;	/* Jump to next block if double block heigt */
						pc += ((jd_yuv_t*)&ix)[0] & (jd_yuv_t)1;				/* Step forward chroma pointer every two pixels */
					...
					/* 修改 */
					/*R*/
					val = yy + (((short)(1.402 * 128) * cr) >> 7);
					if (val < (short)0)
						*pix++ = 0;
					else if (val > (short)255)
						*pix++ = 255;
					else
						*pix++ = val;
					/*G*/
					val = yy - (((short)(0.344 * 128) * cb + (short)(0.714 * 128) * cr) >> 7);
					if (val < (short)0)
						*pix++ = 0;
					else if (val > (short)255)
						*pix++ = 255;
	...
	if (JD_FORMAT == 1) {
		...
		if (jd->swap)
		{
			do {
				...
				((uint8_t*)&w)[1] = (((uint8_t*)&rgb)[0] & (uint8_t)0xf8) | (((uint8_t*)&rgb)[1] >> 5);
				((uint8_t*)&w)[0] = ((((uint8_t*)&rgb)[1] << 3) & (uint8_t)0xE0) | (((uint8_t*)&rgb)[2] >> 3);
				s += 3;

    ...
    case 0x1:  /* PPU */
      switch ( wAddr & 0x7 )
      {
        case 0:    /* 0x2000 */
          ...
          PPU_BG_Base = (PPU_R0 & R0_BG_ADDR) ? ChrBuf + 256 * 8 : ChrBuf; //修改
          PPU_SP_Base = (PPU_R0 & R0_SP_ADDR) ? ChrBuf + 256 * 8 : ChrBuf;
        ...
        case 7:   /* 0x2007 */
          {
            ...
            if ( !( addr & 0xf ) )  /* 0x3f00 or 0x3f10 */
            {
              ...
              //PalTable[ 0x10 ] = PalTable[ 0x14 ] = PalTable[ 0x18 ] = PalTable[ 0x1c ] = NesPalette[ byData ] | 0x8000;  修改
    ...
    case 0x2:  /* Sound */
      switch ( wAddr & 0x1f )
      {
        ...
        case 0x13:
          ...
          //if ( !APU_Mute )
          //  pAPUSoundRegs[ wAddr & 0x1f ]( wAddr, byData ); //修改
          break;
        ...
        case 0x14:  /* 0x4014 */
          ...
          switch ( byData >> 5 )
          {
            ...
            {
                WORD addr = ((WORD)byData << 8) & (WORD)0x7ff;
                BYTE i = 0;
                do {
                    SPRRAM[i] = RAM[addr];
                    SPRRAM[i + (BYTE)1] = RAM[addr + (BYTE)1];
                    SPRRAM[i + (BYTE)2] = RAM[addr + (BYTE)2];
                    SPRRAM[i + (BYTE)3] = RAM[addr + (BYTE)3];
                    SPRRAM[i + (BYTE)4] = RAM[addr + (BYTE)4];
                    SPRRAM[i + (BYTE)5] = RAM[addr + (BYTE)5];
                    SPRRAM[i + (BYTE)6] = RAM[addr + (BYTE)6];
                    SPRRAM[i + (BYTE)7] = RAM[addr + (BYTE)7];
                    i += (BYTE)8;
                    addr += (WORD)8;
                } while (i != (BYTE)0);
            }
        ...
        case 0x15:  /* 0x4015 */
          //InfoNES_pAPUWriteControl( wAddr, byData ); //修改
#if 0
          /* Unknown */
          if ( byData & 0x10 ) 
          {
...
static WORD K6502_ReadW( WORD wAddr ){ return K6502_Read( wAddr ) | (WORD)K6502_Read( wAddr + 1 ) << 8; }
static void K6502_WriteW( WORD wAddr, WORD wData ){ K6502_Write( wAddr, wData & 0xff ); K6502_Write( wAddr + 1, wData >> 8 ); }
static WORD K6502_ReadZpW( BYTE byAddr ){ return K6502_ReadZp( byAddr ) | ( (WORD)K6502_ReadZp( byAddr + 1 ) << 8 ); }

...
/*const*/ struct fsm fsm[] = {
	...
	/* saw <, check <<, <<=, <= */
	LT1,	{ C_XX },	ACT(_LT, S_SELFB),
	LT1,	{ '<' },	LT2,
	LT1,	{ '=' },	ACT(_LEQ, S_SELF),
	LT2,	{ C_XX },	ACT(_LSH, S_SELFB),
	LT2,	{ '=' },	ACT(ASLSH, S_SELF),

	/* saw >, check >>, >>=, >= */
	GT1,	{ C_XX },	ACT(_GT, S_SELFB),
	GT1,	{ '>' },	GT2,
	GT1,	{ '=' },	ACT(_GEQ, S_SELF),
	GT2,	{ C_XX },	ACT(_RSH, S_SELFB),
	GT2,	{ '=' },	ACT(ASRSH, S_SELF),

	/* = */
	ASG1,	{ C_XX },	ACT(_ASGN, S_SELFB),
	ASG1,	{ '=' },	ACT(_EQ, S_SELF),

	/* ! */
	NOT1,	{ C_XX },	ACT(_NOT, S_SELFB),
	NOT1,	{ '=' },	ACT(_NEQ, S_SELF),

	/* & */
	AND1,	{ C_XX },	ACT(_AND, S_SELFB),
	AND1,	{ '&' },	ACT(LAND, S_SELF),
	AND1,	{ '=' },	ACT(ASAND, S_SELF),

	/* | */
	OR1,	{ C_XX },	ACT(_OR, S_SELFB),
	OR1,	{ '|' },	ACT(LOR, S_SELF),
	OR1,	{ '=' },	ACT(ASOR, S_SELF),
...
gettokens(Tokenrow* trp, int reset)
{
	...
	continue2:
		...
		reswitch:
			...
			case S_STNL:
				error(_ERROR, "Unterminated string or char const");
			case S_NL:
				tp->t = ip;
				tp->type = NL;
				tp->len = 1;

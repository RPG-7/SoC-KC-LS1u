...
void tree_init(void)
{
	...
	warn = 0;
	nid = 1;
	for (i = 0; i < 500; ++i) ids[i].printed = 0;
	for (i = 0; i < 500; ++i) ids[i].node = 0;
}
...
Tree tree(int op, Type type, Tree left, Tree right) {
	...
	//NEW0(p, where);
	memset(p = (Tree)allocate(sizeof * p, where), 0, sizeof * p);
	p->op = op;
	p->type = type;
	p->kids[0] = left;
	p->kids[1] = right;
...
static Tree root1(Tree p) {
	...
	case CVF:
		...
		return root1(p->kids[0]);
	case CVI:
		if ((optype(p->op) == U || optype(p->op) == _I)
			&& p->type->size < p->kids[0]->type->size
			&& specific(p->kids[0]->op) != CALL + _I)
			if (warn++ == 0)
				warning("expression with no effect elided\n");
		return root1(p->kids[0]);
	case CVU: case CVP:
		if (optype(p->op) == U && p->type->size < p->kids[0]->type->size
			|| optype(p->op) == _I && p->type->size <= p->kids[0]->type->size)
			if (warn++ == 0)
				warning("expression with no effect elided\n");
		return root1(p->kids[0]);

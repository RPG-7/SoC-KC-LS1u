...
Symbol mkreg(char* fmt, int n, int mask, int set) {
	...
	memset(p->x.regnode = (Regnode)allocate(sizeof * p->x.regnode, PERM), 0, sizeof * p->x.regnode);
...
int range(Node p, int lo, int hi) {
	...
	case CNST + _I:  ck(s->u.c.v.i >= lo && s->u.c.v.i <= hi);
...
/*驱动prelabel, _label和ralloc,rewrite为单个树选择相应的指令*/
...
/*gen选择代码,在dag上添加注释以记录选择结果,并返回一个dag指针; 驱动rewrite, prune, linearize, ralloc, 处理代码表中的各个森林，70*/
Node gen(Node forest) {
	...
	for (p = forest; p; p = p->link) {
		...
		if (generic(p->op) == CALL)
			...
			(*IR->x.doarg)(p);
		rewrite(p); //驱动prelabel, _label和ralloc,rewrite为单个树选择相应的指令
		p->x.listed = 1;
	}
	for (p = forest; p; p = p->link)
		prune(p, &dummy); //从树中剔除某些子指令
	relink(&sentinel, &sentinel);
	for (p = forest; p; p = p->link)
		linearize(p, &sentinel); //为输出排序指令
...
static void genspill(Symbol r, Node last, Symbol tmp) {
	...
	//NEW0(s, FUNC);
	memset(s = (Symbol)allocate(sizeof * s, FUNC), 0, sizeof * s);

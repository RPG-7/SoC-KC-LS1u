...
static void asgncode(Type ty, int lev) {
	...
	ty->x.marked = 1;
	switch (ty->op) {
	case _VOLATILE: case _CONST: case _VOLATILE + _CONST:
		asgncode(ty->type, lev);
		ty->x.typeno = ty->type->x.typeno;
		break;
	case _POINTER: case _FUNCTION: case _ARRAY:
...
static int emittype(Type ty, int lev, int col) {
	...
	ty->x.printed = 1;
	switch (ty->op) {
	case _VOID:	/* void is defined as itself */
		print("=%d", tc), col += 1 + 3;
		break;
	case _INT:
	...
	case _UNSIGNED:
		...
		break;
	case _FLOAT:	/* float, double, long double get sizes, not ranges */
	...
	case _FUNCTION:
		...
		col = emittype(ty->type, lev + 1, col);
		break;
	case _ARRAY:	/* array includes subscript as an int range */
	...
	case _STRUCT: case _UNION: {
		Field p;
		if (!ty->u.sym->defined) {
			print("=x%c%s:", ty->op == _STRUCT ? 's' : 'u', ty->u.sym->name);
		...
		print("=%c%d", ty->op == _STRUCT ? 's' : 'u', ty->size), col += 1 + 1 + 3;
		for (p = fieldlist(ty); p; p = p->link) {
			if (p->name)

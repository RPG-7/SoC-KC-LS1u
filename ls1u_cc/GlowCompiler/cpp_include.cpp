...
void cpp_include_init(void)
{
	for (int i = 0; i < NINCLUDE; ++i)
		includelist[i].always = 0;
	for (int i = 0; i < NINCLUDE; ++i)
		includelist[i].deleted = 0;
	for (int i = 0; i < NINCLUDE; ++i)
		includelist[i].file = 0;
	ta = { UNCLASS };
	tr = { &ta, &ta, &ta + 1, 1 };
}
...
doinclude(Tokenrow* trp)
{
	...
	if (trp->tp >= trp->lp)
		goto syntax;
	if (trp->tp->type != STRING && trp->tp->type != _LT) {
		len = trp->tp - trp->bp;
	...
	else if (trp->tp->type == _LT) {
		len = 0;
		trp->tp++;
		while (trp->tp->type != _GT) {
	...
	if (fd != NULL) {
		...
		trp->tp = trp->bp + 2;
		error(_ERROR, "Could not find include file %r", trp);
	}
	return;
syntax:
	error(_ERROR, "Syntax error in #include");
	return;
}

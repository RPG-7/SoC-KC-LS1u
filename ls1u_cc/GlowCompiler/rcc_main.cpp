...
int rcc_main(int argc, char* argv[])
{
	...
	for (i = argc - 1; i > 0; i--)  if (strncmp(argv[i], "-target=", 8) == 0) break;

	if (i > 0)
	{
		char* s = strchr(argv[i], '\\');
		if (s != NULL) *s = '/';
		for (j = 0; bindings[j].name && bindings[j].ir; j++) if (strcmp(&argv[i][8], bindings[j].name) == 0)
		{
			IR = bindings[j].ir;
			break;
		}
	...
	if (!IR)
	{
		...
		if (i > 0) fprint(stderr, " `%s'", &argv[i][8]);
	...
	if (glevel && IR->_stabinit)
		(*IR->_stabinit)(firstfile, argc, argv);
	...
	if (glevel || xref) {
		...
		src.x = 0;
		src.y = lineno;
		if ((glevel > 2 || xref) && IR->_stabend)
			(*IR->_stabend)(&src, symroot,
				(Coordinate**)ltov(&loci, PERM),
				(Symbol*)ltov(&symbols, PERM), NULL);
		else if (IR->_stabend)
			(*IR->_stabend)(&src, NULL, NULL, NULL, NULL);
	}
	finalize();
	(*IR->_progend)();
	deallocate(PERM);
	...
	for (i = 1; i < argc; i++)
		if (strcmp(argv[i], "-g") == 0 || strcmp(argv[i], "-g2") == 0)
			...
			if (p) {
				...
				if (glevel == 0)
					glevel = 1;
				if (stabIR._stabline == NULL) {
					stabIR._stabline = IR->_stabline;
					stabIR._stabend = IR->_stabend;
					IR->_stabline = stabline;
					IR->_stabend = stabend;
				}
			}
		}
...
static void typestab(Symbol p, void* cl) {
	if (*(Symbol*)cl == 0 && p->sclass && p->sclass != TYPEDEF)
		*(Symbol*)cl = p;
	if ((p->sclass == TYPEDEF || p->sclass == 0) && IR->_stabtype)
		(*IR->_stabtype)(p);
}
...
static void stabend(Coordinate* cp, Symbol p, Coordinate** cpp, Symbol* sp, Symbol* stab) {
	if (stabIR._stabend)
		(*stabIR._stabend)(cp, p, cpp, sp, stab);
	if (srcfp)
		fclose(srcfp);
}

...
struct value {
	//long val;
	long long val; //修改支持64位,2022.3.10
	int	type;
};
...
void cpp_eval_init(void)
{
	vp = 0;
	op = 0;
	for (int i = 0; i < NSTAK; ++i)
		vals[i].type = 0;
	for (int i = 0; i < NSTAK; ++i)
		vals[i].val = 0;
	for (int i = 0; i < NSTAK; ++i)
...
eval(Tokenrow* trp, int kw)
{
	...
	if (kw == KIFDEF || kw == KIFNDEF) {
		if (trp->lp - trp->bp != 4 || trp->tp->type != NAME) {
			error(_ERROR, "Syntax error in #ifdef/#ifndef");
			return 0;
		}
		np = lookup(trp->tp, 0);
	...
	*op++ = END;
	for (rand = 0, tp = trp->bp + ntok; tp < trp->lp; tp++) {
		...
		case STRING:
			if (rand)
				goto syntax;
			if (vp == &vals[NSTAK]) {
				error(_ERROR, "Eval botch (stack overflow)");
				return 0;
			}
			*vp++ = tokval(tp);
			rand = 1;
		...
		case _NOT:
			...
			*op++ = (toktype)tp->type;
			...
			/* unary-binary */
		case PLUS: case MINUS: case STAR: case _AND:
			if (rand == 0) {
				if (tp->type == MINUS)
					*op++ = UMINUS;
				if (tp->type == STAR || tp->type == _AND) {
					error(_ERROR, "Illegal operator * or & in #if/#elsif");
					return 0;
				}
				continue;
			}
		...
		/* plain binary */
		case _EQ: case _NEQ: case _LEQ: case _GEQ: case _LSH: case _RSH:
		case LAND: case LOR: case SLASH: case PCT:
		case _LT: case _GT: case CIRC: case _OR: case QUEST:
		...
		default:
			error(_ERROR, "Bad operator (%t) in #if/#elsif", tp);
			return 0;
		}
	}
	if (rand == 0)
	...
	if (op != &ops[1] || vp != &vals[1]) {
		error(_ERROR, "Botch in #if/#elsif");
		return 0;
	}
	if (vals[0].type == UND)
		error(_ERROR, "Undefined expression value");
	return vals[0].val;
syntax:
	error(_ERROR, "Syntax error in #if/#elsif");
	return 0;
}
...
evalop(struct pri pri)
{
	...
	while (pri.pri < priority[op[-1]].pri) {
		...
		switch (oper) {
		case _EQ: case _EQ | UNSMARK:
			rv1 = rv1 == rv2; break;
		case _NEQ: case _NEQ | UNSMARK:
			rv1 = rv1 != rv2; break;
		case _LEQ:
			rv1 = rv1 <= rv2; break;
		case _GEQ:
		...
		case _GT:
			rv1 = rv1 > rv2; break;
		case _LEQ | UNSMARK:
			rv1 = (unsigned long long)rv1 <= rv2; break;
		case _GEQ | UNSMARK:
			rv1 = (unsigned long long)rv1 >= rv2; break;
		case _LT | UNSMARK:
			rv1 = (unsigned long long)rv1 < rv2; break;
		...
		case DEFINED:
			break;
		default:
			error(_ERROR, "Eval botch (unknown operator)");
			return 1;
		}
		/*lint +e574 +e644 */
		v1.val = rv1;
	...
	tokval(Token* tp)
{
	...
	int i, base, c;
	//unsigned long n;
	unsigned long long n; //修改支持64位,2022.3.10
	uchar* p;
	...
	case NUMBER:
		...
		if (n >= 0x8000000000000000 && base != 10) //修改支持64位,2022.3.10
			v.type = UNS;
		for (; *p; p++) {
			if (*p == 'u' || *p == 'U')
				v.type = UNS;
	...
	case CCON:
		...
		if (*p == '\\') {
			...
			error(_ERROR, "Empty character constant");
		else
			n = *p++;
		if (*p != '\'')
	...
	case STRING:
		error(_ERROR, "String in #if/#elsif");
		break;
	}
	return v;
}

...
static void bbcall(Symbol yycounts, Coordinate* cp, Tree* e) {
	...
	(*IR->_defconst)(U, unsignedtype->size, (v.u = u.coord, v));
...
static void bbincr(Symbol yycounts, Coordinate* cp, Tree* e) {
	...
	if (mp->size >= NELEMS(mp->u)) {
		//NEW(mp, PERM);
		mp = (struct map*)allocate(sizeof * mp, PERM);
		mp->size = 0;
...
static void bbvars(Symbol yylink, void* ignore, void* ignore2) {
	...
	if (YYcounts) {
		...
		(*IR->_space)(YYcounts->type->size);
	}
	files = genident(STATIC, array(charptype, 1, 0), GLOBAL);
	defglobal(files, LIT);
	for (p = (Symbol*)ltov(&filelist, PERM); *p; p++)
		defpointer((*p)->u.c.loc);
	defpointer(NULL);
	coords = genident(STATIC, array(unsignedtype, n, 0), GLOBAL);
	defglobal(coords, LIT);
	for (i = n, mp = (struct map**)ltov(&maplist, PERM); *mp; i -= (*mp)->size, mp++)
		for (j = 0; j < (*mp)->size; j++)
			(*IR->_defconst)(U, unsignedtype->size, (v.u = (*mp)->u[j].coord, v));
	if (i > 0)
		(*IR->_space)(i * coords->type->type->size);
	(*IR->_defconst)(U, unsignedtype->size, (v.u = 0, v));
	defglobal(yylink, DATA);
	defpointer(NULL);
	(*IR->_defconst)(U, inttype->size, (v.u = n, v));

...
void defpointer(Symbol p) {
	if (p) {
		...
		//static Value v;
		(*IR->_defconst)(P, voidptype->size, v);
	}
}
...
static int genconst(Tree e, int def) {
	for (;;)
		...
		case CNST:
			...
			if (def)
				(*IR->_defconst)(e->type->op, e->type->size, e->u.v);
			return e->type->size;
		case RIGHT:
...
static int initchar(int len, Type ty) {
	...
	do {
		*s++ = initvalue(ty)->u.v.i;
		if (++n % inttype->size == 0) {
			(*IR->_defstring)(inttype->size, buf);
			s = buf;
		}
		if (len > 0 && n >= len || t != ',')
			break;
...
static int initfields(Field p, Field q) {
	...
	for (i = 0; i < n; i++) {
		...
		(*IR->_defconst)(U, unsignedchar->size, v);
	}
	return n;
}
...
Type initializer(Type ty, int lev) {
	...
	Tree e;
	Type aty = NULL;
	static char follow[] = { IF, _CHAR, STATIC, 0 };
	...
	if ((isunion(ty) || isstruct(ty)) && ty->size == 0) {
		static char follow[] = { _CHAR, STATIC, 0 };
	...
	if (isarray(ty) && ischar(aty)) {
		if (t == SCON) {
			...
			n = tsym->type->size;
			(*IR->_defstring)(tsym->type->size, (char*)tsym->u.c.v.p);
			t = gettok();
		}
		else if (t == '{') {
			t = gettok();

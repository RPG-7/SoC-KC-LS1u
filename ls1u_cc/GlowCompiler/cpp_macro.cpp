...
*/
static unsigned char one[2];
static Token onetoken[1];
static Tokenrow onetr;

static Token t;
static Tokenrow tr;


void cpp_macro_init(void)
{
	one[0] = '1';
	one[1] = '\0';
	onetoken[0] = {NUMBER, 0, 0, 0, 1, one};
	onetr = { onetoken, onetoken, onetoken + 1, 1 };
	t = { STRING };
	tr = { &t, &t, &t + 1, 1 };
}
...
dodefine(Tokenrow* trp)
{
	...
	if (tp >= trp->lp || tp->type != NAME) {
		error(_ERROR, "#defined token is not a name");
		return;
	}
	np = lookup(tp, 1);
	if (np->flag & ISUNCHANGE) {
		error(_ERROR, "#defined token %t can't be redefined", tp);
		return;
	}
	/* collect arguments */
	tp += 1;
	args = NULL;
	if (tp < trp->lp && tp->type == _LP && tp->wslen == 0) {
		/* macro with args */
		int narg = 0;
		tp += 1;
		args = new(Tokenrow);
		...
		if (tp->type != RP) {
			...
			for (;;) {
				...
				for (atp = args->bp; atp < args->lp; atp++)
					if (atp->len == tp->len
						...
						error(_ERROR, "Duplicate macro argument");
				*args->lp++ = *tp;
				narg++;
				tp += 1;
				if (tp->type == RP)
			...
			if (err) {
				error(_ERROR, "Syntax error in macro parameters");
				return;
			}
		}
		tp += 1;
	...
	if (np->flag & ISDEFINED) {
		if (comparetokens(def, np->vp)
			...
			error(_ERROR, "Macro redefinition of %t", trp->bp + 2);
	}
	if (args) {
		Tokenrow* tap;
	...
	for (tp = trp->tp; tp < trp->lp; ) {
		...
		if (np->val == KDEFINED) {
			...
			if ((tp + 1) < trp->lp && (tp + 1)->type == NAME)
				(tp + 1)->type = NAME1;
			else if ((tp + 3) < trp->lp && (tp + 1)->type == _LP
				&& (tp + 2)->type == NAME && (tp + 3)->type == RP)
				(tp + 2)->type = NAME1;
			else
				error(_ERROR, "Incorrect syntax for `defined'");
			tp++;
			continue;
		}
		if (np->flag & ISMAC)
...
expand(Tokenrow* trp, Nlist* np)
{
	...
	if (np->ap == NULL)			/* parameterless */
		...
		if (narg != rowlen(np->ap)) {
			error(_ERROR, "Disagreement in number of macro arguments");
	...
	*narg = 0;
	...
	while (parens > 0) {
		...
		if (trp->tp->type == END) {
			trp->lp -= 1;
			trp->tp -= ntok;
			error(_ERROR, "EOF in macro arglist");
			return ntok;
		}
		if (trp->tp->type == NL) {
			trp->tp += 1;
	...
	for (; parens >= 0; lp++) {
		...
		if (lp->type == COMMA && parens == 0 || parens < 0 && (lp - 1)->type != _LP) {
			if (*narg >= NARG - 1)
	...
	for (rtr->tp = rtr->bp; rtr->tp < rtr->lp; ) {
		if (rtr->tp->type == SHARP) {	/* string operator */
			...
			if ((argno = lookuparg(np, rtr->tp)) < 0) {
				error(_ERROR, "# not followed by macro parameter");
				continue;
			}
			ntok = 1 + (rtr->tp - tp);
			rtr->tp = tp;
	...
	for (trp->tp = trp->bp; trp->tp < trp->lp; trp->tp++) {
		if (trp->tp->type == DSHARP1)
			...
			if (ltp < trp->bp || ntp >= trp->lp) {
				error(_ERROR, "## occurs at border of replacement");
				continue;
			}
			len = ltp->len + ntp->len;
	...
	*sp++ = '"';
	for (tp = vp->bp; tp < vp->lp; tp++) {
		...
		if (sp + 2 * tp->len >= &s[STRLEN - 10]) {
			error(_ERROR, "Stringified macro arg is too long");
			break;
		}
...
builtin(Tokenrow* trp, int biname)
{
	...
	*op++ = '"';
	...
	default:
		error(_ERROR, "cpp botch: unknown internal macro");
		return;
	}
	if (tp->type == STRING)
		*op++ = '"';

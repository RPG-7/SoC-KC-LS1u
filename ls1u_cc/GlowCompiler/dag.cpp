...
static struct dag* dagnode(int op, Node l, Node r, Symbol sym) {
	...
	//NEW0(p, FUNC);
	memset(p = (dag*)allocate(sizeof * p, FUNC), 0, sizeof * p);
...
/*将分析树e转换为dag,并将dag添加到森林中,143，243*/
Node listnodes(Tree tp, int tlab, int flab) {
	...
	case CALL: { Tree save = firstarg;
		...
		//NEW0(p->syms[0], FUNC);
		memset(p->syms[0] = (Symbol)allocate(sizeof * p->syms[0], FUNC), 0, sizeof* p->syms[0]);
...
/*解释代码表,并将树传递给gen; 遍历前端的私有数据结构,将dag的每个森林传递给后端过程gen,70, 276*/
void gencode(Symbol caller[], Symbol callee[]) {
	...
	for (; errcnt <= 0 && cp; cp = cp->next)
		switch (cp->kind) {
		case code::Address:  assert(IR->_address);
			(*IR->_address)(cp->u.addr.sym, cp->u.addr.base,
				cp->u.addr.offset); break;
		case code::Blockbeg: {
			Symbol* p = cp->u.block.locals;
			(*IR->_blockbeg)(&cp->u.block.x);
			for (; *p; p++)
				if ((*p)->ref != 0.0)
					(*IR->_local)(*p);
				else if (glevel) (*IR->_local)(*p);
		}
					 break;
		case code::Blockend: (*IR->_blockend)(&cp->u.begin->u.block.x); break;
		case code::Defpoint: src = cp->u.point.src; break;
		case code::Gen: case code::Jump:
		case code::Label:    if (prunetemps)
			cp->u.forest = prune(cp->u.forest);
			fixup(cp->u.forest);
			cp->u.forest = (*IR->_gen)(cp->u.forest); break;
		case code::Local:    (*IR->_local)(cp->u.var); break;
...
void emitcode(void) {
	...
	{
		switch (cp->kind) {
		case code::Address: break;
		case code::Blockbeg: if (glevel && IR->_stabblock) {
			(*IR->_stabblock)('{', cp->u.block.level - LOCAL, cp->u.block.locals);
			swtoseg(CODE);
		}
						   break;
		case code::Blockend: if (glevel && IR->_stabblock) {
			Code bp = cp->u.begin;
			foreach(bp->u.block.identifiers, bp->u.block.level, typestab, NULL);
			foreach(bp->u.block.types, bp->u.block.level, typestab, NULL);
			(*IR->_stabblock)('}', bp->u.block.level - LOCAL, bp->u.block.locals);
			swtoseg(CODE);
		}
						   break;
		case code::Defpoint: src = cp->u.point.src;
			if (glevel > 0 && IR->_stabline) {
				(*IR->_stabline)(&cp->u.point.src); swtoseg(CODE);
			} break;
		case code::Gen: case code::Jump:
		case code::Label:    if (cp->u.forest)
			(*IR->_emit)(cp->u.forest); break;
		case code::Local:    if (glevel && IR->_stabsym) {
			(*IR->_stabsym)(cp->u.var);
			swtoseg(CODE);
		} break;
		case code::Switch: {	int i;
			defglobal(cp->u.swtch.table, LIT);
			(*IR->_defaddress)(equated(cp->u.swtch.labels[0]));
			for (i = 1; i < cp->u.swtch.size; i++) {
				//long k = cp->u.swtch.values[i - 1];
				long long k = cp->u.swtch.values[i - 1]; //修改支持64位,2022.2.27
				while (++k < cp->u.swtch.values[i])
					assert(k < LONGLONG_MAX), //修改支持64位,2022.2.27
					//assert(k < LONG_MAX),
					(*IR->_defaddress)(equated(cp->u.swtch.deflab));
				(*IR->_defaddress)(equated(cp->u.swtch.labels[i]));
			}
			swtoseg(CODE);
		} break;
		default: assert(0);
...
static void typestab(Symbol p, void* cl) {
	if (!isfunc(p->type) && (p->sclass == EXTERN || p->sclass == STATIC) && IR->_stabsym)
		(*IR->_stabsym)(p);
	else if ((p->sclass == TYPEDEF || p->sclass == 0) && IR->_stabtype)
		(*IR->_stabtype)(p);
}

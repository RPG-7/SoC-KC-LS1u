...
extern _Interface sparcIR, solarisIR;
...
/*
generated at Thu Feb  4 16:25:26 2021
by $Id$
*/
...
static void _closure_reg(NODEPTR_TYPE a, int c) {
	struct _state* p = (_state*)a->x.state;
...
static void _label(NODEPTR_TYPE a) {
	...
	STATE_LABEL(a) = p = (_state*)allocate(sizeof * p, FUNC);
	p->rule._stmt = 0;
	p->cost[1] =
		p->cost[2] =
		p->cost[3] =
	...
	case 1093: /* INDIRI1 */
		...
		if (	/* reg: INDIRI1(VREGP) */
			...
			if (mayrecalc(a)) {
				struct _state* q = (_state*)a->syms[RX]->u.t.cse->x.state;
...
static void target(Node p) {
	...
	case CNST + _I: case CNST + U: case CNST + P:
		if (range(p, 0, 0) == 0) {
			setreg(p, greg[0]);
			p->x.registered = 1;
		}
...
static void clobber(Node p) {
	assert(p);
	switch (specific(p->op)) {
	case CALL + B: case CALL + F: case CALL + _I:
...
static void defconst(int suffix, int size, Value v) {
	if (suffix == F && size == 4) {
		...
		print(".byte 0x%x\n", (unsigned)((unsigned char)(suffix == _I ? v.i : v.u)));
	else if (size == 2)
		print(".half 0x%x\n", (unsigned)((unsigned short)(suffix == _I ? v.i : v.u)));
	else if (size == 4)
		print(".word 0x%x\n", (unsigned)(suffix == _I ? v.i : v.u));
	else assert(0);
}

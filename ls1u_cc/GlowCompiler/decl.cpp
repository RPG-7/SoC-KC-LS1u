...
static Type specifier(int* sclass) {
	...
	for (;;) {
		...
		case _CONST:    p = &cons; t = gettok();      break;
		case _VOLATILE: p = &vol;  t = gettok();      break;
		case SIGNED:
		case _UNSIGNED: p = &sign; t = gettok();      break;
		case _LONG:     if (size == _LONG) {
			size = 0;
			tt = _LONG + _LONG;
		}
				 p = &size; t = gettok();      break;
		case _SHORT:    p = &size; t = gettok();      break;
		case _VOID: case _CHAR: case _INT: case _FLOAT:
		case _DOUBLE:   p = &type; ty = tsym->type;
			t = gettok();      break;
		case _ENUM:     p = &type; ty = enumdcl();    break;
		case _STRUCT:
		case _UNION:    p = &type; ty = structdcl(t); break;
		case ID:
	...
	if (size == _SHORT && type != _INT
		|| size == _LONG + _LONG && type != _INT
		|| size == _LONG && type != _INT && type != _DOUBLE
		|| sign && type != _INT && type != _CHAR)
		error("invalid type specification\n");
	if (type == _CHAR && sign)
		ty = sign == _UNSIGNED ? unsignedchar : signedchar;
	else if (size == _SHORT)
		ty = sign == _UNSIGNED ? unsignedshort : shorttype;
	else if (size == _LONG && type == _DOUBLE)
		ty = longdouble;
	else if (size == _LONG + _LONG) {
		ty = sign == _UNSIGNED ? unsignedlonglong : longlong;
		if (Aflag >= 1)
			warning("`%t' is a non-ANSI type\n", ty);
	}
	else if (size == _LONG)
		ty = sign == _UNSIGNED ? unsignedlong : longtype;
	else if (sign == _UNSIGNED && type == _INT)
		ty = unsignedtype;
	if (cons == _CONST)
		ty = qual(_CONST, ty);
	if (vol == _VOLATILE)
...
static void decl(Symbol(*dcl)(int, char*, Type, Coordinate*)) {
	int sclass; //值为83表示有typedef前缀
	Type ty, ty1;
	static char stop[] = { _CHAR, STATIC, ID, 0 };
...
static Symbol dclglobal(int sclass, char* id, Type ty, Coordinate* pos) {
	...
	if (t == '=' && isfunc(p->type)) {
		...
		initglobal(p, 0);
		if (glevel > 0 && IR->_stabsym) {
			(*IR->_stabsym)(p); swtoseg(p->u.seg);
		}
	}
...
static Type tnode(int op, Type type) {
	...
	//NEW0(ty, STMT);
	memset(ty = (Type)allocate(sizeof * ty, PERM), 0, sizeof * ty);
...
static Type dclr1(char** id, Symbol** params, int abstract) {
	...
	case ID:                if (id)
		*id = token;
		...
		t = gettok(); break;
	case '*': t = gettok(); if (t == _CONST || t == _VOLATILE) {
		Type ty1;
		ty1 = ty = tnode(t, NULL);
		while ((t = gettok()) == _CONST || t == _VOLATILE)
			ty1 = tnode(t, ty1);
...
static Symbol* parameters(Type fty) {
	...
	if (kind[t] == STATIC || istypename(t, tsym)) {
		...
		fty->u.f.proto = (Type*)newarray(length(list) + 1,
			sizeof(Type*), PERM);
		params = (Symbol*)ltov(&list, FUNC);
		for (n = 0; params[n]; n++)
	...
	if (t != ')') {
		static char stop[] = { _CHAR, STATIC, IF, ')', 0 };
		expect(')');
		skipto('{', stop);
	}
	if (t == ')')
...
static void funcdefn(int sclass, char* id, Type ty, Symbol params[], Coordinate pt) {
	...
	if (ty->u.f.oldstyle) {
		...
		caller = params;
		callee = (Symbol*)newarray(n + 1, sizeof * callee, FUNC);
		...
		if (p && p->scope == GLOBAL && isfunc(p->type)
			...
			Type* proto = (Type*)newarray(n + 1, sizeof * proto, PERM);
			if (Aflag >= 1)
		...
		callee = params;
		caller = (Symbol*)newarray(n + 1, sizeof * caller, FUNC);
		for (i = 0; (p = callee[i]) != NULL && p->name; i++) {

			//NEW(caller[i], FUNC);
			caller[i] = (Symbol)allocate(sizeof * caller[i], FUNC);
	...
	if (!IR->wants_callb && isstruct(rty)) {
		Symbol* a;
		a = (Symbol*)newarray(n + 2, sizeof * a, FUNC);
		a[0] = retv;
		...
		//NEW(a[0], FUNC);
		a[0] = (Symbol)allocate(sizeof * a[0], FUNC);
	...
	if (cfunc->sclass != STATIC)
		(*IR->_export)(cfunc);
	if (glevel && IR->_stabsym) {
		swtoseg(CODE); (*IR->_stabsym)(cfunc);
	}
	swtoseg(CODE);
	(*IR->_function)(cfunc, caller, callee, cfunc->u.f.ncalls);
	if (glevel && IR->_stabfend)
		(*IR->_stabfend)(cfunc, lineno);
...
void compound(int loop, struct swtch* swp, int lev) {
	...
	if (level == LOCAL) {
		Code cp;
		for (cp = codelist; cp->kind < code::Label; cp = cp->prev)
			;
		if (cp->kind != code::Jump) {
	...
	code(code::Blockend)->u.begin = cp;
	if (reachable(code::Gen))
		definept(NULL);
	if (level > LOCAL) {
		exitscope();
		expect('}');
...
void checkref(Symbol p, void* cl) {
	if (p->scope >= PARAM
...
static Symbol dcllocal(int sclass, char* id, Type ty, Coordinate* pos) {
	...
	case EXTERN:   q = lookup(id, globals);
		if (q == NULL || q->sclass == TYPEDEF || q->sclass == _ENUM) {
		...
		p->u.alias = q; break;
	case STATIC:   (*IR->_defsymbol)(p);
		initglobal(p, 0);
		if (!p->defined)
			if (p->type->size > 0) {
				defglobal(p, BSS);
				(*IR->_space)(p->type->size);
			}
			else
				error("undefined size for `%t %s'\n",
					p->type, p->name);
	...
	if (p->u.c.loc) {
		...
		if (isarray(p->type) && p->type->type == widechar) {
			...
			while (n-- > 0) {
				Value v;
				v.u = *s++;
				(*IR->_defconst)(widechar->op, widechar->size, v);
			}
		}
		else if (isarray(p->type))
			(*IR->_defstring)(p->type->size,(char*) p->u.c.v.p);
		else
			(*IR->_defconst)(p->type->op, p->type->size, p->u.c.v);
		p->u.c.loc = NULL;
	}
}
...
Type enumdcl(void) {
	...
	if (t == '{') {
		...
		ty->u.sym->u.idlist = (Symbol*)ltov(&idlist, PERM);
		ty->u.sym->defined = 1;
	}
	else if ((p = lookup(tag, types)) != NULL && p->type->op == _ENUM) {
		ty = p->type;
		if (t == ';')
			error("empty declaration\n");
	}

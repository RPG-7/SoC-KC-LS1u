...
enum {
	/*BLANK非换行空白字符集合,
	LINE只含换行符,LETTER大小写字母集合,DIGIT包含0-9集合,HEX十六进制集合,OTHER包含运算符括号等*/
...
/* 175 }   */	OTHER,
/* 176 ~   */	OTHER, };
static struct symbol tval;  //提供常量类型和值
static char cbuf[BUFSIZE + 1];  //字符串缓冲区
...
int gettok(void) {
	for (;;) {
		...
		case 'L':
			if (*rcp == '\'') {
				unsigned int* s = (unsigned int*)scon(*cp, wcput, wcbuf);
				if (s - wcbuf > 2)
...
static Symbol icon(unsigned long long n, int overflow, int base) {
	if ((*cp == 'u' || *cp == 'U') && (cp[1] == 'l' || cp[1] == 'L') && (cp[2] == 'l' || cp[2] == 'L')
		...
		if(overflow || n > unsignedlong->u.sym->u.limits.max.u)
			tval.type = unsignedlonglong;
		else if(n > unsignedshort->u.sym->u.limits.max.u)
			tval.type = unsignedlong;
		else if (n > unsignedtype->u.sym->u.limits.max.u)
	...
	else if (overflow || n > longlong->u.sym->u.limits.max.i)
		tval.type = unsignedlonglong;
	else if (base != 10 && n > inttype->u.sym->u.limits.max.i && n <= unsignedtype->u.sym->u.limits.max.i)
		tval.type = unsignedtype;
	else if (n > longtype->u.sym->u.limits.max.i)
		tval.type = longlong;
	else
		tval.type = inttype;
	switch (tval.type->op) {
...
static Symbol icon(unsigned long long n, int overflow, int base) {
	if ((*cp == 'u' || *cp == 'U') && (cp[1] == 'l' || cp[1] == 'L') && (cp[2] == 'l' || cp[2] == 'L')
		...
		tval.type = longlong;
	else if (n > shorttype->u.sym->u.limits.max.i)
		tval.type = longtype;
	else if (n > chartype->u.sym->u.limits.max.i)
		tval.type = shorttype;
...
Symbol icon(unsigned long n, int overflow, int base) {
...
static Symbol fcon(void) {
	...
	errno = 0;
	tval.u.c.v.d = strtod(token, NULL);  //字符串转浮点
	if (errno == ERANGE)
...
/*c的值装入cl指向的单元,返回值为cl加1*/
static void* wcput(int c, void* cl) {
	unsigned int* s = (unsigned int*)cl;

	*s++ = c;
	return s;
}

/*处理 ' 和 "  ,q为 ' 或 " ,put选择缓冲区处理函数,cl选择缓冲区,返回值为cl*/
static void* scon(int q, void* put(int c, void* cl), void* cl) {
	int n = 0, nbad = 0;
	...
	do {
		...
		while (*cp != q) {
			int c;
			if (map[*cp] & NEWLINE) {        //遇到换行符进入处理
				if (cp < limit)  //缓冲区没空，跳出循环,进入报错程序
					break;
				cp++;
				nextline();
				if (cp == limit)    //缓冲区空,跳出循环,进入报错程序
					break;
				continue;  //不是换行,是缓冲区尾,补充好缓冲区了,从头来过
			}
			c = *cp++;
			if (c == '\\') {                 //遇到'\\'进入处理
				if (map[*cp] & NEWLINE) {
					if (cp++ < limit)
						continue;     //是换行符,跳过换行,从头来过
					nextline();  //不是换行符,是缓冲区尾
				}
				...
				nbad++;
			if (n++ < BUFSIZE)
				cl = put(c, cl);  //c的值装入cl指向的单元,返回值为cl加1
		}
		if (*cp == q)
			cp++;
		else
...
/*跳过BLANK空白字符和NEWLINE换行,返回下一个首字符*/
int getchr(void) {
	for (;;) {
		while (map[*cp] & BLANK)
			cp++;
